...
export default function AdminlarPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!editing && !form.password) { setFormErr(t("foydalanuvchilarAdminlar.passwordRequired")); return }
  ...
  return (
    <section className="flex flex-col min-h-full" style={{ backgroundColor: "#f6f9ff" }}>
      <header className="flex flex-col gap-[15px] pt-[25px] pb-5 px-5 bg-white" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
        <h1 className="font-medium text-[28px]" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("foydalanuvchilarAdminlar.title")}</h1>
      </header>
        ...
        <div className="bg-white rounded-[5px] overflow-hidden" style={{ border: "1px solid rgba(1,41,112,0.1)", boxShadow: "0px 0px 5px rgba(1,41,112,0.1)" }}>
          <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              <h2 className="font-medium text-[22px]" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("foydalanuvchilarAdminlar.allAdmins")}</h2>
            ...
            <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
              <label className="w-full px-2.5 py-2 rounded-[5px] border flex items-center sm:w-[400px]" style={{ borderColor: "rgba(1,41,112,0.3)" }}>
              ...
              <button onClick={openAdd} className="flex items-center justify-center gap-2 h-[42px] px-[15px] rounded-[5px] text-white text-base" style={{ backgroundColor: "#0e58a8", fontFamily: "var(--font-poppins)" }}>
                <Plus className="w-5 h-5" /> {t("foydalanuvchilarAdminlar.addBtn")}
              </button>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
                  {[t("foydalanuvchilarUmumiy.colIndex"), t("foydalanuvchilarUmumiy.colFio"), t("foydalanuvchilarUmumiy.colUsername"), t("foydalanuvchilarUmumiy.colPhone"), t("foydalanuvchilarAdminlar.colRole"), t("foydalanuvchilarUmumiy.colStatus"), t("foydalanuvchilarUmumiy.colAction")].map((h) => (
                  ...
                  <motion.tr key={a.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }}
                    ...
                    <td className="px-4 h-14">
                      <div className="inline-flex items-center gap-1.5 h-[30px] px-3 rounded-[10px] border" style={{ backgroundColor: a.role === "super_admin" ? "#fff8e6" : "#f6f9ff", borderColor: a.role === "super_admin" ? "#f59e0b" : "#1cc2dc" }}>
                        ...
                        <span className="text-xs font-semibold" style={{ color: a.role === "super_admin" ? "#f59e0b" : "#1cc2dc", fontFamily: "var(--font-poppins)" }}>{a.role === "super_admin" ? t("foydalanuvchilarAdminlar.roleSuperAdmin") : t("foydalanuvchilarAdminlar.roleAdmin")}</span>
                      </div>
                    </td>
                    <td className="px-4 h-14">
                      <span className="px-3 py-1 rounded-full text-xs font-semibold" style={{ backgroundColor: a.status === "active" ? "#f0fbfd" : "#f6f9ff", color: a.status === "active" ? "#1cc2dc" : "#7293b9", border: `1px solid ${a.status === "active" ? "#1cc2dc" : "#7293b9"}` }}>
                        {a.status === "active" ? t("foydalanuvchilarUmumiy.statusActive") : t("foydalanuvchilarUmumiy.statusInactive")}
                      </span>
                    </td>
                      ...
                      <AnimatePresence>
                        {openId === a.id && (
                          <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -4 }} transition={{ duration: 0.15 }}
                            ...
                            <button className="flex items-center gap-2 w-full px-4 py-2.5 text-sm hover:bg-[#f6f9ff]" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }} onClick={() => { setOpenId(null); openEdit(a) }}><Pencil className="w-4 h-4" /> {t("foydalanuvchilarUmumiy.edit")}</button>
                            <button className="flex items-center gap-2 w-full px-4 py-2.5 text-sm hover:bg-red-50" style={{ color: "#ef4444", fontFamily: "var(--font-poppins)" }} onClick={() => { setOpenId(null); handleDelete(a.id) }}><Trash2 className="w-4 h-4" /> {t("foydalanuvchilarUmumiy.delete")}</button>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </td>
      ...
      <Modal open={modal} title={editing ? t("foydalanuvchilarAdminlar.modalTitleEdit") : t("foydalanuvchilarAdminlar.addBtn")} onClose={closeModal}>
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FSelect label={t("foydalanuvchilarAdminlar.colRole")} value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>
              <option value="admin">{t("foydalanuvchilarAdminlar.roleAdmin")}</option>
              <option value="super_admin">{t("foydalanuvchilarAdminlar.roleSuperAdmin")}</option>
            </FSelect>
