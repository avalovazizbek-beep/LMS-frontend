...
export default function UstalarPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!editing && !form.password) { setFormErr(t("foydalanuvchilarUstalar.passwordRequired")); return }
  ...
  return (
    <section className="flex flex-col min-h-full" style={{ backgroundColor: "#f6f9ff" }}>
      <header className="flex flex-col gap-[15px] pt-[25px] pb-5 px-5 bg-white" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
        <h1 className="font-medium text-[28px]" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("foydalanuvchilarUstalar.title")}</h1>
      </header>

      <div className="grid grid-cols-1 gap-5 px-[30px] pt-[30px] sm:grid-cols-3">
        {[
          { label: t("foydalanuvchilarUstalar.statTotal"),     value: masters.length,                                            color: "#012970" },
          { label: t("foydalanuvchilarUmumiy.statusActive"),   value: masters.filter((m) => m.status === "active").length,   color: "#22c55e" },
          { label: t("foydalanuvchilarUmumiy.statusInactive"), value: masters.filter((m) => m.status === "inactive").length, color: "#ef4444" },
        ].map((s) => (
        ...
        <div className="bg-white rounded-[5px] overflow-hidden" style={{ border: "1px solid rgba(1,41,112,0.1)", boxShadow: "0px 0px 5px rgba(1,41,112,0.1)" }}>
          <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              <h2 className="font-medium text-[22px]" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("foydalanuvchilarUstalar.allMasters")}</h2>
            ...
            <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
              ...
              <button onClick={openAdd} className="flex items-center justify-center gap-2 h-[42px] px-[15px] rounded-[5px] text-white text-sm" style={{ backgroundColor: "#0e58a8", fontFamily: "var(--font-poppins)" }}>
                <Plus className="w-5 h-5" /> {t("foydalanuvchilarUstalar.addBtn")}
              </button>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
                  {[t("foydalanuvchilarUmumiy.colIndex"), t("foydalanuvchilarUmumiy.colFio"), t("foydalanuvchilarUmumiy.colUsername"), t("foydalanuvchilarUmumiy.colPhone"), t("foydalanuvchilarUmumiy.colStatus"), t("foydalanuvchilarUmumiy.colAction")].map((h) => (
      ...
      <Modal open={modal} title={editing ? t("foydalanuvchilarUstalar.modalTitleEdit") : t("foydalanuvchilarUstalar.addBtn")} onClose={closeModal}>
