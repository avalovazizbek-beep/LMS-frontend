...
export default function Imtihonlar() {
  ...
  const SECTIONS: Array<{ key: ContentStatus; label: string; emptyText: string }> = [
    { key: "open",   label: t("exams.section.open"),   emptyText: t("exams.sectionEmpty.open") },
    { key: "locked", label: t("exams.section.locked"), emptyText: t("exams.sectionEmpty.locked") },
    { key: "closed", label: t("exams.section.closed"), emptyText: t("exams.sectionEmpty.closed") },
  ]
  ...
  return (
    <div className="flex flex-col gap-6 p-[30px]">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-[28px] font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("exams.title")}</h1>
      ...
      <div className="grid grid-cols-3 gap-5">
        {[
          { label: t("exams.current"),  value: grouped.open.length,   color: "#15803d", Icon: Clock },
          { label: t("exams.upcoming"), value: grouped.locked.length, color: "#b91c1c", Icon: Lock },
          { label: t("exams.finished"), value: grouped.closed.length, color: "#92400e", Icon: CheckCircle2 },
        ].map((s, i) => (
