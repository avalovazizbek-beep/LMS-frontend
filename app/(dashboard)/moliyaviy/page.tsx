  ...
  if (!contract) {
    return (
      <div className="flex flex-col gap-6 p-[30px]">
        <div>
          <h1 className="text-[28px] font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("moliyaviy.title")}</h1>
          <p className="text-sm mt-1" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("moliyaviy.subtitle")}</p>
        </div>
        <div className="bg-white rounded-[10px] p-10 text-center" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
          ...
          <p className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("moliyaviy.notFound")}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-[30px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {[
          { label: t("moliyaviy.totalPayment"),   value: formatSum(totalAmount), icon: Wallet,       color: "#012970" },
          { label: t("moliyaviy.paid"),           value: formatSum(paidAmount),  icon: CheckCircle2, color: "#22c55e" },
          { label: t("moliyaviy.remainingDebt"),  value: formatSum(debtAmount),  icon: TrendingDown, color: debtAmount > 0 ? "#ef4444" : "#22c55e" },
        ].map(s => {
          const Icon = s.icon
          return (
      ...
      <div className="bg-white rounded-[10px] p-5" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("moliyaviy.paymentProgress")}</h2>
        ...
        <div className="flex justify-between mt-2 text-xs" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
          <span>{t("moliyaviy.paidLabel", { sum: formatSum(paidAmount) })}</span>
          <span>{t("moliyaviy.totalLabel", { sum: formatSum(totalAmount) })}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-[10px] p-5" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("moliyaviy.contractInfo")}</h2>
          </div>
          {[
            { label: t("moliyaviy.contractNo"),   value: contract.contractNumber },
            { label: t("moliyaviy.status"),       value: contract.status },
            { label: t("moliyaviy.stage"),        value: contract.course },
            { label: t("moliyaviy.direction"),    value: contract.speciality },
            { label: t("moliyaviy.totalSum"),     value: formatSum(contract.contractAmount) },
            { label: t("moliyaviy.paid"),         value: formatSum(contract.paidAmount) },
            { label: t("moliyaviy.debt"),         value: formatSum(contract.debitAmount) },
            { label: t("moliyaviy.lastPayment"),  value: contract.lastPaymentDate ?? "—" },
          ].map(item => (
        ...
        <div className="bg-white rounded-[10px] p-5 flex flex-col gap-4" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-base font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("moliyaviy.actions")}</h2>
          </div>
          {contract.pdfLink && (
          ...
          <div className="mt-2 p-4 rounded-[8px]" style={{ backgroundColor: "#f6f9ff" }}>
            ...
            <p className="text-xs mt-1" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
              {debtAmount > 0 ? t("moliyaviy.remaining", { sum: formatSum(debtAmount) }) : t("moliyaviy.fullyPaid")}
            </p>
          </div>
        </div>
      </div>
