...
export default function OqituvchiMurojaatlar() {
  ...
  if (selectedId !== null) {
    return (
      <div className="flex flex-col gap-4 p-[30px] h-[calc(100vh-64px)]">
        <ConversationThread
  ...
  return (
    <div className="flex flex-col gap-5 p-[30px]">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-[28px] font-medium" style={T}>Murojaatlar</h1>
          <p className="text-sm mt-1" style={L}>Talabalardan kelgan murojaatlar</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[10px] p-16 text-center" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
          <MessageSquareText className="w-10 h-10 mx-auto mb-3" style={{ color: "#d8e6f7" }} />
          <p className="text-sm font-semibold" style={T}>Hozircha murojaat yo'q</p>
        </div>
      ) : (
          ...
          {items.map(c => (
            <button key={c.id} onClick={() => setSelectedId(c.id)}
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs mt-1 truncate" style={L}>{c.subject} · {fmtDate(c.lastMessageAt)}</div>
              </div>
              {c.status === "open" ? (
