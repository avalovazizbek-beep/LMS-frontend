"use client"

import { useEffect, useState } from "react"
import { TrendingUp, RefreshCw } from "lucide-react"
import { useLanguage } from "@/lib/i18n/LanguageContext"
import { reeduApi, type ReeduGradeType } from "@/lib/api"

const GRADE_TYPES: ReeduGradeType[] = ["JN", "ON1", "ON2", "YN"]

type MyEnrollment = Awaited<ReturnType<typeof reeduApi.me>>["data"][number]

export default function QaytaOqishOzlashtirish() {
  const { t } = useLanguage()
  const [enrollments, setEnrollments] = useState<MyEnrollment[] | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    reeduApi.me().then(res => setEnrollments(res.data)).finally(() => setLoading(false))
  }, [])

  return (
    <div className="flex flex-col gap-6 p-[30px]">
      <div>
        <h1 className="text-[28px] font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>
          {t("sidebar.item.retakePerformance")}
        </h1>
        <p className="text-sm mt-1" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
          {t("retakePerf.subtitle")}
        </p>
      </div>

      {loading ? (
        <div className="flex items-center justify-center py-20">
          <RefreshCw className="w-5 h-5 animate-spin" style={{ color: "#0e58a8" }} />
        </div>
      ) : !enrollments || enrollments.length === 0 ? (
        <div className="bg-white rounded-[10px] p-14 text-center"
          style={{ border: "1px solid rgba(1,41,112,0.1)", boxShadow: "0px 0px 5px rgba(1,41,112,0.05)" }}>
          <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
            style={{ backgroundColor: "#f0fff4" }}>
            <TrendingUp className="w-8 h-8" style={{ color: "#22c55e" }} />
          </div>
          <p className="text-base font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>
            {t("retakePerf.notFound")}
          </p>
          <p className="text-sm mt-2 max-w-sm mx-auto" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
            {t("retakePerf.noneYet")}
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          {enrollments.map(e => (
            <div key={e.id} className="bg-white rounded-[10px] overflow-hidden" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
              <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
                <div>
                  <h2 className="text-base font-semibold" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{e.subjectName}</h2>
                  <p className="text-xs" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{e.groupName}</p>
                </div>
                <span className="text-sm font-semibold" style={{ color: e.finalScore !== null ? (e.finalScore >= 55 ? "#22c55e" : "#ef4444") : "#7293b9" }}>
                  {e.finalScore !== null ? t("qaytaOqish.finalScore", { score: e.finalScore }) : t("qaytaOqish.grading")}
                </span>
              </div>
              <div className="grid grid-cols-4 gap-3 px-5 py-4">
                {GRADE_TYPES.map(gt => {
                  const g = e.grades.find(gr => gr.gradeType === gt)
                  return (
                    <div key={gt} className="rounded-[8px] p-3 text-center" style={{ backgroundColor: "#f6f9ff" }}>
                      <div className="text-xs font-medium" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{gt}</div>
                      <div className="text-lg font-semibold mt-1" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{g?.grade ?? "—"}</div>
                    </div>
                  )
                })}
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  )
}
