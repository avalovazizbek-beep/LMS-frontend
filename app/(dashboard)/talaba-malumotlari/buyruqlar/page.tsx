...
export default function Buyruqlar() {
  ...
  return (
    <div className="flex flex-col gap-6 p-[30px]">
      <div>
        <h1 className="text-[28px] font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("buyruqlar.title")}</h1>
        <p className="text-sm mt-1" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("buyruqlar.subtitle")}</p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {decrees.length === 0 ? (
          <div className="bg-white rounded-[10px] p-10 text-center" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
            ...
            <p className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("buyruqlar.notFound")}</p>
          </div>
        ) : decrees.map(d => (
