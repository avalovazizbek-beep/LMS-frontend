...
export default function TalabaHujjat() {
  ...
  const docTypeLabels: Record<string, string> = {
    academic_sheet: t("talabaHujjat.docType.academicSheet"),
    academic_data:  t("talabaHujjat.docType.academicData"),
    "call-sheet":   t("talabaHujjat.docType.callSheet"),
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-[30px]">
      <div>
        <h1 className="text-[28px] font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("talabaHujjat.title")}</h1>
        <p className="text-sm mt-1" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("talabaHujjat.subtitle")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {docs.length === 0 ? (
          <div className="col-span-2 bg-white rounded-[10px] p-10 text-center" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
            ...
            <p className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("talabaHujjat.notFound")}</p>
          </div>
        ) : docs.map((d, i) => (
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-sm" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>
                  {d.name ?? docTypeLabels[d.type ?? ""] ?? t("talabaMalumotlari.document")}
                </p>
                {d.type && (
