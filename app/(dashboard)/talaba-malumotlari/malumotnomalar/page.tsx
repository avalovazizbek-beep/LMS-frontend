...
import { hemisApi, HemisReference, hemisDownloadUrl } from "@/lib/api"
...
export default function Malumotnomalar() {
  ...
  return (
    <div className="flex flex-col gap-6 p-[30px]">
      <div>
        <h1 className="text-[28px] font-medium" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>{t("malumotnomalar.title")}</h1>
        <p className="text-sm mt-1" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("malumotnomalar.subtitle")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {refs.length === 0 ? (
          <div className="col-span-3 bg-white rounded-[10px] p-10 text-center" style={{ border: "1px solid rgba(1,41,112,0.1)" }}>
            ...
            <p className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>{t("malumotnomalar.notFound")}</p>
          </div>
        ) : refs.map((r, i) => (
          <div key={`${r.id}-${i}`} className="bg-white rounded-[10px] p-4 flex flex-col gap-3"
            ...
            <div>
              <p className="font-medium text-sm" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>
                {r.name ?? t("malumotnomalar.reference")}
              </p>
              {r.created_at && (
            ...
            {r.file && (
              <a href={hemisDownloadUrl(r.file, r.name ?? "ma'lumotnoma")} download
