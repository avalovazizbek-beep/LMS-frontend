...
export default function ReeduGroupPage() {
  ...
  const TABS: { key: Tab; label: string; icon: typeof CalendarDays }[] = [
    { key: "jadval", label: "3. Dars jadvali", icon: CalendarDays },
    { key: "davomat", label: "4. Davomat", icon: ClipboardCheck },
    { key: "nazorat", label: "5. Nazorat", icon: BookCheck },
    { key: "qaydnoma", label: "6. Qaydnoma", icon: FileCheck2 },
  ]
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
            {detail.group.subjectName} · {detail.enrollments.length} talaba · {detail.group.status === "active" ? "Faol" : "Yopilgan"}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b" style={{ borderColor: "rgba(1,41,112,0.1)" }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            ...
            style={{
              borderColor: tab === t.key ? "#0e58a8" : "transparent",
              color: tab === t.key ? "#0e58a8" : "#7293b9",
  ...
  return (
    <Card>
      <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
        <h2 className="text-base font-semibold" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>Haftalik dars jadvali</h2>
      ...
      <div className="p-5 flex flex-col gap-3">
        {slots.length === 0 && <p className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>Hali jadval belgilanmagan</p>}
        {slots.map((s, i) => (
          <div key={i} className="flex items-center gap-2 flex-wrap">
            <select value={s.weekDay} onChange={e => update(i, "weekDay", Number(e.target.value))}
              ...
              {WEEK_DAYS.map((d, idx) => <option key={d} value={idx + 1}>{d}</option>)}
            </select>
            ...
            <input type="time" value={s.endTime} onChange={e => update(i, "endTime", e.target.value)}
              className="px-3 py-2 rounded-[8px] text-sm outline-none" style={{ border: "1px solid rgba(1,41,112,0.15)", color: "#012970", fontFamily: "var(--font-poppins)" }} />
            <input value={s.room} onChange={e => update(i, "room", e.target.value)} placeholder="Xona"
...
function AttendanceTab({ id, detail, onSaved }: { id: number; detail: ReeduGroupDetail; onSaved: () => void }) {
  ...
  const STATUS_OPTS = [
    { v: "present", label: "Keldi", color: "#22c55e" },
    { v: "late", label: "Kechikdi", color: "#f59e0b" },
    { v: "excused", label: "Sababli", color: "#0891b2" },
    { v: "absent", label: "Kelmadi", color: "#ef4444" },
  ]

  return (
    <Card>
      <div className="px-5 py-4 flex items-center justify-between gap-3 flex-wrap" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
        <h2 className="text-base font-semibold" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>Davomat belgilash</h2>
        ...
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(1,41,112,0.08)", backgroundColor: "#f6f9ff" }}>
              {["Talaba", "Holat", "Umumiy davomat"].map(h => (
  ...
  return (
    <Card>
      <div className="px-5 py-4" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
        <h2 className="text-base font-semibold" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>Nazorat baholari</h2>
        <p className="text-xs mt-0.5" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
          Har bir baho 0-100 oralig'ida. Yakuniy ball — JN/ON1/ON2/YN o'rtachasi (55 va undan yuqori bo'lsa "tugallandi").
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(1,41,112,0.08)", backgroundColor: "#f6f9ff" }}>
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "#1cc2dc", fontFamily: "var(--font-poppins)" }}>Talaba</th>
              {GRADE_TYPES.map(gt => (
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "#1cc2dc", fontFamily: "var(--font-poppins)" }}>Holat</th>
              ...
              <tr key={e.studentUserId} style={{ borderBottom: "1px solid rgba(1,41,112,0.06)" }}>
                ...
                <td className="px-4 py-3">
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{
                    ...
                    {e.status === "completed" ? `Tugallandi (${e.finalScore})` : e.status === "failed" ? `Yiqildi (${e.finalScore})` : "Davom etmoqda"}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <button onClick={() => finalize(e.studentUserId)} disabled={finalizing === e.studentUserId}
                    ...
                    {finalizing === e.studentUserId ? "…" : "Yakunlash"}
                  </button>
                </td>
              </tr>
            ))}
...
function RecordTab({ id, detail }: { id: number; detail: ReeduGroupDetail }) {
  ...
  return (
    <Card>
      <div className="px-5 py-4" style={{ borderBottom: "1px solid rgba(1,41,112,0.1)" }}>
        <h2 className="text-base font-semibold" style={{ color: "#012970", fontFamily: "var(--font-poppins)" }}>Qaydnoma — HEMIS uchun hisobot</h2>
        <p className="text-xs mt-0.5" style={{ color: "#92400e", fontFamily: "var(--font-poppins)" }}>
          HEMIS hozircha tashqi tizimlardan avtomatik yozishga (POST) ruxsat bermaydi — shu sabab quyidagi hisobotni
          HEMIS operatoriga (dekanat) qo'lda kiritish uchun tayyorlanadi.
        </p>
      </div>
      {loading ? (
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(1,41,112,0.08)", backgroundColor: "#f6f9ff" }}>
                  {["Talaba", "ID raqami", "Fan", "Asl guruh", "Qarzdorlik ball", "Yakuniy ball", "Holat"].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "#1cc2dc", fontFamily: "var(--font-poppins)" }}>{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {(rows ?? []).map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid rgba(1,41,112,0.06)" }}>
                    ...
                    <td className="px-4 py-3">
                      ...
                      }}>{r.readyForHemis ? "Tayyor" : "Kutilmoqda"}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="p-5 flex items-center gap-2">
            ...
            <span className="text-sm" style={{ color: "#7293b9", fontFamily: "var(--font-poppins)" }}>
              {readyCount} / {rows?.length ?? 0} talaba yakuniy hisobotga tayyor
            </span>
          </div>
        </>
      )}
